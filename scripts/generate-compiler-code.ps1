...
function Build-Tools() {
    ...
    try {
        foreach ($item in $list) { 
            ...
            Exec-Command $dotnet "publish /p:Configuration=$config /v:m `"$proj`"" | Out-Null

            $exePath = GetProjectPublishedBinary $exeName -configuration=$config -projectName:$fileName -tfm:$tfm -published:$true
