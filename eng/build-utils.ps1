...
function GetProjectOutputBinary([string]$fileName, [string]$projectName = $null, [string]$configuration = $script:configuration, [string]$tfm = "net472", [bool]$published = $false) {
  $projectName = if ($projectName -ne $null) { $projectName } else { Split-Path -LeafBase $fileName }
  $publishDir = if ($published) { "publish\" } else { "" }
  return Join-Path $ArtifactsDir "$projectName\$configuration\$tfm\$publishDir$fileName"
}
